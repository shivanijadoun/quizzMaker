...
export default function CreateQuiz() {
  ...
  return (
    <div className="flex flex-row items-center p-4 bg-teal-300 min-h-screen">
      ...
      <div className='flex flex-col items-center p-4 bg-teal-950 border rounded-xl m-3 overflow-hidden'>
        <input
          type="text"
          placeholder="Question"
          value={currentQuestion}
          onChange={(e) => setCurrentQuestion(e.target.value)}
          className="mb-2 p-2 border rounded w-96 bg-teal-700 text-white"
        />
        {options.map((option, index) => (
          <input
            ...
            className="mb-2 p-2 border rounded bg-teal-700 text-white"
          />
        ))}
        <input
          type="text"
          ...
          className="mb-2 p-2 border rounded bg-teal-600 text-white"
        />
        <button onClick={handleAddQuestion} className="px-4 py-2 bg-teal-900 text-white rounded">
          Add Question
        </button>
        <button onClick={handleSaveQuiz} className="mt-4 px-4 py-2 bg-teal-900 text-white rounded">
          Save Quiz
        </button>
        </div>
      </div>
